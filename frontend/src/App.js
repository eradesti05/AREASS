...
function InnerApp({ user, setUser, tasks, setTasks }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#EFF1F5", fontFamily: "'Segoe UI', sans-serif" }}>
      {user && (
        <Navbar
          user={user}
      ...
      <Routes>
        ...
        <Route path="/dashboard" element={
          user?.role === "mahasiswa" ? <DashboardMahasiswa user={user} /> : <Navigate to="/login" />
        } />
        ...
        <Route path="/analytics" element={
          user?.role === "mahasiswa" ? <AnalyticsPage /> : <Navigate to="/login" />
        } />
        <Route path="/dashboard-dosen" element={
          user?.role === "dosen_wali" ? <DashboardDosenWali /> : <Navigate to="/login" />
        } />
        <Route path="/dashboard-kaprodi" element={
          user?.role === "kaprodi" ? <DashboardKaprodi /> : <Navigate to="/login" />
        } />
...
function App() {
  ...
  return (
    <BrowserRouter>
      <InnerApp user={user} setUser={setUser} tasks={tasks} setTasks={setTasks} />
    </BrowserRouter>
  );
}
